      ...
      <div className="max-w-[1320px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end mb-12">
          <div className="lg:col-span-7">
            <Reveal direction="up">
              <span className="inline-flex items-center gap-2 text-domenion-gold font-heading text-xs font-extrabold tracking-widest uppercase">CONTACT INFORMATION</span>
            </Reveal>
              ...
              <div className="bg-neutral-light border border-neutral-border hover:border-domenion-gold/50 rounded-xl p-6 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col justify-between h-full group">
                <div>
                  <div className="flex items-center justify-between pb-3 mb-3 border-b border-neutral-border">
                    <span className="text-domenion-gold font-heading text-xs font-extrabold tracking-widest uppercase">{card.label}</span>
                  ...
                  <h3 className="text-domenion-blue font-heading text-lg font-bold mb-1 break-all">{card.value}</h3>
                  <p className="text-gray-500 text-xs leading-relaxed">{card.subtext}</p>
                </div>
              </div>
            );
            ...
            return (
              <div key={card.label}>
                <Reveal direction="up" delay={0.06 * idx}>
                  {card.href ? (
                    <a href={card.href} className="block text-decoration-none h-full">
                      {content}
                    </a>
                  ) : (
                    content
