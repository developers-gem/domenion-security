...
export default function ContactLocation() {
  return (
    <section className="py-20 sm:py-28 bg-neutral-light text-domenion-blue border-b border-neutral-border" id="location-section">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-6">
            <div className="flex flex-col">
              <Reveal direction="up">
                <span className="inline-flex items-center gap-2 text-domenion-gold font-heading text-xs font-extrabold tracking-widest uppercase">SERVICE COVERAGE</span>
              </Reveal>
                ...
                {COVERED_REGIONS.map((region, idx) => (
                  <Reveal key={region} direction="up" delay={0.05 * idx}>
                    <div className="flex items-center gap-2.5 p-3 rounded-lg bg-white border border-neutral-border text-xs font-semibold text-domenion-blue">
                      <CheckCircle2 size={15} className="text-domenion-gold flex-shrink-0" />
                      <span>{region}</span>
                    </div>
                  </Reveal>
                ))}
