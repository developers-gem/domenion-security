...
function AdminApplications() {
  ...
  const [selectedApp, setSelectedApp] = useState(null);
  const [modalTab, setModalTab] = useState("quiz"); // "quiz" | "employment"
  const [deleteConfirmId, setDeleteConfirmId] = useState(null);
  const [actionLoading, setActionLoading] = useState(false);
  ...
  const handleOpenDetails = (app, initialTab = "quiz") => {
    setSelectedApp(app);
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  {filteredApps.map((item) => (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="inline-flex items-center gap-1.5">
                          {/* BUTTON 1: QUIZ DETAILS */}
                          <button
                            type="button"
                            onClick={() => handleOpenDetails(item, "quiz")}
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-blue-200 bg-blue-50/60 hover:bg-blue-100 text-blue-700 font-medium text-xs shadow-xs transition-colors"
                            title="View Quiz & Screening Answers"
                          >
                            <HelpCircle size={13} />
                            <span>Quiz</span>
                          </button>

                          {/* BUTTON 2: EMPLOYMENT FORM DETAILS */}
                          <button
                            type="button"
                            onClick={() =>
                              handleOpenDetails(item, "employment")
                            }
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-amber-200 bg-amber-50/60 hover:bg-amber-100 text-amber-800 font-medium text-xs shadow-xs transition-colors"
                            title="View Full Employment Form"
                          >
                            <FileCheck size={13} />
